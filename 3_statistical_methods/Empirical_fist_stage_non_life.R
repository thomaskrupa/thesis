...
### This code perform the statistical methods for the non-life insurance sector. First paths should be adjusted.
...
setwd("C:/Users/tkrup/Documents/solution")
...
# Overall efficiency
 Path <- "C:/Users/tkrup/Documents/phd thesis/7_ratio_MA_graphics/non_life/1_overall/1_CRS"

# Production efficiency
# Path <- "C:/Users/tkrup/Documents/phd thesis/7_ratio_MA_graphics/non_life/2_div1/1_CRS"

# Investment efficiency
# Path <- "C:/Users/tkrup/Documents/phd thesis/7_ratio_MA_graphics/non_life/3_div2/1_CRS"
...
# Overall efficiency
# Path <- "C:/Users/tkrup/Documents/phd thesis/7_ratio_MA_graphics/non_life/1_overall/2_VRS"
...
# Scale efficiency
# Path <- "C:/Users/tkrup/Documents/phd thesis/7_ratio_MA_graphics/non_life/4_Scale"
...
b1 <- ggplot(dfx2, aes(y = dfx2, x = a)) +
  ...
  labs(x ="", y="Ratio")+
  scale_x_discrete()+
  scale_y_continuous(limits=c(-0.25,2.5), breaks=seq(-0.25,2.5,0.25))+
  ggtitle("Non-acquirer")+
...
# non-acquirer vs acquirer before
leveneTest(k$Ratio, k$AC)

# non-acquirer vs acquirer after
leveneTest(l$Ratio, l$AC)

###########################################
# Parametric and non-parametric two-sample tests
###########################################
...
describeBy(Ratio~AC, data = k)
 ...
 #  non-acquirer vs acquirer before excl.
 # t test
 t.test(Ratio~AC, data=k, var.equal = TRUE, alternative = "less")
 # T test bootstrap
 set.seed(1315)
 boot.t.test(dfx2$Ratio, dfy2$Ratio, alternative ="less", paired = FALSE, mu = 0, var.equal = TRUE,
            conf.level = 0.95, R = 10000)
 # Welch test
 t.test(Ratio~AC, data=k, var.equal = FALSE, alternative = "less")
 # Welch test bootstrap
 set.seed(1315)
 boot.t.test(dfx2$Ratio, dfy2$Ratio, alternative ="less", paired = FALSE, mu = 0, var.equal = FALSE,
             conf.level = 0.95, R = 10000)
 # Mann-Whitney U test
 wilcox.test(Ratio~AC, data=k, exact=FALSE, alternative ="less")

 #  non-acquirer vs acquirer after excl.
 # means sich anschauen
 ...
 # Welch test bootstrap
 set.seed(1315)
 boot.t.test(dfx2$Ratio, dfz2$Ratio, alternative ="greater", paired = FALSE, mu = 0, var.equal = FALSE,
             conf.level = 0.95, R = 10000)
 # Mann-Whitney U test
