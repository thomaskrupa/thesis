...
test_that("Test case 222",{


  total_wide <- sum(1,K*N, sum_r,sum_m);
  total_height <- sum(1,sum_m,sum_r, K, 2*sum_l,1);




  A <- Obj.func_and_con(1, direction, Input, Output, Link, Amount_Input, Amount_Output, Amount_Link, K, N, sum_m, sum_r, sum_l, weights, Link_obj)$FCON;
  ...
  G <- t_constraint(K, N, sum_m, sum_r, sum_l, Link_obj);

  f.con_ <- rbind(A, B, C, D, E, G);
  ...
  #Constraint 1
  expect_equal(dim(Obj.func_and_con(1, direction, Input, Output, Link, Amount_Input, Amount_Output, Amount_Link, K, N, sum_m, sum_r, sum_l, weights, Link_obj)$FCON)[2], total_wide, check.attributes = FALSE) ;
  ...
  #t Constraint
  expect_equal(dim(t_constraint(K, N, sum_m, sum_r, sum_l, Link_obj))[2], total_wide, check.attributes = FALSE)
