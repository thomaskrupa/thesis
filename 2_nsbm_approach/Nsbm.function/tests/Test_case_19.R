...
test_that("Test case 19",{
  ...
  #Constraint 5:
  Const_5_i1 <- matrix(c(-10,10,4,8,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -5,5,7,4,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -10,rep(0,N),10,4,8,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -5,rep(0,N),5,7,4,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -1,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -4,rep(0,N),4,3,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -1,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),
                         -4,rep(0,((N-1)*K)),4,3,2,rep(0,sum_r),rep(0,sum_m)), byrow = TRUE, nrow = 8);

  Const_5_i2 <- matrix(c(-4,10,4,8,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -7,5,7,4,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -4,rep(0,N),10,4,8,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -7,rep(0,N),5,7,4,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -5,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -3,rep(0,N),4,3,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -5,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),
                         -3,rep(0,((N-1)*K)),4,3,2,rep(0,sum_r),rep(0,sum_m)), byrow = TRUE, nrow = 8);

  Const_5_i3 <- matrix(c(-8,10,4,8,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -4,5,7,4,rep(0,((N-1)*K)),rep(0,sum_r),rep(0,sum_m),
                         -8,rep(0,N),10,4,8,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -4,rep(0,N),5,7,4,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -2,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -2,rep(0,N),4,3,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),
                         -2,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),
                         -2,rep(0,((N-1)*K)),4,3,2,rep(0,sum_r),rep(0,sum_m)), byrow = TRUE, nrow = 8);
