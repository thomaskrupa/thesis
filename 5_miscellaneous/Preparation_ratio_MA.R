...
#Life
 ...
 # Life CRS
 # Path <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/life/1_CRS"
 
 # Life path VRS
 # Path <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/life/2_VRS"

# Scale efficiency life
 #Path <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/life/scale"

# Control matrix für non-acquirer
# Path_con <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/life/12_Control.csv"
...
# Non-life path CRS
# Path <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/non_life/1_CRS"


# Non-life path VRS
# Path <- "C:/Users/tkrup/Documents/phd thesis/6_Preparation_ratio_MA/non_life/2_VRS"
...
for(i in seq_along(file_contents)){
  ...
  for(j in 1:7){
    ...
    if(j == 1){
      res_non_acq <- t(t(non_acq[,1]));
      res_acq_b_excl <- t(t(acq_b_excl[,1]));
      res_acq_a_excl <- t(t(acq_a_excl[,1]));
    }else{
      res_non_acq <- rbind(res_non_acq, t(t(non_acq[,1])));
      res_acq_b_excl <- rbind(res_acq_b_excl, t(t(acq_b_excl[,1])));
      res_acq_a_excl <- rbind(res_acq_a_excl, t(t(acq_a_excl[,1])));
    }
  }
  H1 <-paste("eff_non_acq", "_", toString(i),".csv", sep="")
  write.table(res_non_acq, file = H1, row.names = FALSE, dec = ".", sep = ";", quote = FALSE)

  H2 <-paste("eff_acq_b", "_", toString(i),".csv", sep="")
  write.table(res_acq_b_excl, file = H2, row.names = FALSE, dec = ".", sep = ";", quote = FALSE)

  H3 <-paste("eff_acq_a", "_", toString(i),".csv", sep="")
