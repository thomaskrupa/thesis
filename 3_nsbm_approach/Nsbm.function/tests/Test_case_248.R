...
Data_1 <- matrix(c(3, 5, 8, 1, 2, 4,
                   7, 1, 4, 5, 8, 3,
                   2, 5, 3, 2, 1, 2),byrow=TRUE, nrow=3);
...
test_that("Test case 248",{


  #objective function and Constraint1:
  Const_1_i1 <- matrix(c(1,rep(0,N),rep(0,N),(w1/(r1+l1))*(1/5),(w2/(r1))*(1/1),rep(0,sum_m),(w1/(r1+l1))*(1/2),(w1/(r1+l1))*(1/4),rep(0,sum_l)), byrow = TRUE, nrow = 1);
  Const_1_i2 <- matrix(c(1,rep(0,N),rep(0,N),(w1/(r1+l1))*(1/1),(w2/(r1))*(1/5),rep(0,sum_m),(w1/(r1+l1))*(1/8),(w1/(r1+l1))*(1/3),rep(0,sum_l)), byrow = TRUE, nrow = 1);
  Const_1_i3 <- matrix(c(1,rep(0,N),rep(0,N),(w1/(r1+l1))*(1/5),(w2/(r1))*(1/2),rep(0,sum_m),(w1/(r1+l1))*(1/1),(w1/(r1+l1))*(1/2),rep(0,sum_l)), byrow = TRUE, nrow = 1);

  obj_func_i1 <- matrix(c(1,rep(0,N),rep(0,N),rep(0,sum_r),-(w1/(m1))*(1/3),-(w2/(m1+l1))*(1/8),rep(0,sum_l),-(w2/(m1+l1))*(1/2),-(w2/(m1+l1))*(1/4)), byrow = TRUE, nrow = 1);
  obj_func_i2 <- matrix(c(1,rep(0,N),rep(0,N),rep(0,sum_r),-(w1/(m1))*(1/7),-(w2/(m1+l1))*(1/4),rep(0,sum_l),-(w2/(m1+l1))*(1/8),-(w2/(m1+l1))*(1/3)), byrow = TRUE, nrow = 1);
  obj_func_i3 <- matrix(c(1,rep(0,N),rep(0,N),rep(0,sum_r),-(w1/(m1))*(1/2),-(w2/(m1+l1))*(1/3),rep(0,sum_l),-(w2/(m1+l1))*(1/1),-(w2/(m1+l1))*(1/2)), byrow = TRUE, nrow = 1);
  ...
  #Constraint 5:
  Const_5_i1 <- matrix(c(-2,2,8,1,rep(0,N),rep(0,sum_m),rep(0,sum_r),-1,0,0,0,
                         -4,4,3,2,rep(0,N),rep(0,sum_m),rep(0,sum_r),0,-1,0,0,
                         -2,rep(0,N),2,8,1,rep(0,sum_m),rep(0,sum_r),0,0,1,0,
                         -4,rep(0,N),4,3,2,rep(0,sum_m),rep(0,sum_r),0,0,0,1), byrow = TRUE, nrow = 4);

  Const_5_i2 <- matrix(c(-8,2,8,1,rep(0,N),rep(0,sum_m),rep(0,sum_r),-1,0,0,0,
                         -3,4,3,2,rep(0,N),rep(0,sum_m),rep(0,sum_r),0,-1,0,0,
                         -8,rep(0,N),2,8,1,rep(0,sum_m),rep(0,sum_r),0,0,1,0,
                         -3,rep(0,N),4,3,2,rep(0,sum_m),rep(0,sum_r),0,0,0,1), byrow = TRUE, nrow = 4);

  Const_5_i3 <- matrix(c(-1,2,8,1,rep(0,N),rep(0,sum_m),rep(0,sum_r),-1,0,0,0,
                         -2,4,3,2,rep(0,N),rep(0,sum_m),rep(0,sum_r),0,-1,0,0,
                         -1,rep(0,N),2,8,1,rep(0,sum_m),rep(0,sum_r),0,0,1,0,
                         -2,rep(0,N),4,3,2,rep(0,sum_m),rep(0,sum_r),0,0,0,1), byrow = TRUE, nrow = 4);
  ...
  weightsNSBM <- matrix(c(1,rep(2,(K*N)),rep(3,(sum_r)), rep(4,(sum_m)), rep(5,(sum_l)), rep(6,(sum_l)),
                          1,rep(2,(K*N)),rep(3,(sum_r)), rep(4,(sum_m)), rep(5,(sum_l)), rep(6,(sum_l)),
                          1,rep(2,(K*N)),rep(3,(sum_r)), rep(4,(sum_m)), rep(5,(sum_l)), rep(6,(sum_l))), byrow = TRUE, nrow = 3);

  # slack_plus
  slack_plus <- matrix(c(rep(3,(sum_r)),rep(5,(sum_l)),
                         rep(3,(sum_r)),rep(5,(sum_l)),
                         rep(3,(sum_r)),rep(5,(sum_l))), byrow = TRUE, nrow = 3);

  # slack_minus
  slack_minus <- matrix(c(rep(4,(sum_r)),rep(6,(sum_l)),
                          rep(4,(sum_r)),rep(6,(sum_l)),
                          rep(4,(sum_r)),rep(6,(sum_l))), byrow = TRUE, nrow = 3);
  ...
  nsbm.div_res <- matrix(c(  (-1/3)/(147/60), (-16/24)/4,
                             (3/7)/(199/72), (-18/72)/(8/5),
                             -1/(111/30), (-44/18)/(5/2)) , byrow = TRUE, nrow = 3);
