...
Data_1 <- matrix(c(7,2,3,1,5,4,6,2,7,5,8,2,1,
                   4,3,8,1,2,3,1,1,3,1,2,2,5,
                   1,8,7,4,5,7,5,4,6,4,7,1,2),byrow=TRUE, nrow=3);
...
test_that("Test case 261",{


  #objective function and Constraint1:
  Const_1_i1 <- matrix(c(1,rep(0,(K*N)),(w1/(r1+l1))*(1/3),(w1/(r1+l1))*(1/1),(w2/(r2+l2))*(1/6),(w2/(r2+l2))*(1/2),(w3/(r3))*(1/8),(w3/(r3))*(1/2),rep(0,sum_m),(w2/(r2+l2))*(1/1),rep(0,sum_l)), byrow = TRUE, nrow = 1);
  Const_1_i2 <- matrix(c(1,rep(0,(K*N)),(w1/(r1+l1))*(1/8),(w1/(r1+l1))*(1/1),(w2/(r2+l2))*(1/1),(w2/(r2+l2))*(1/1),(w3/(r3))*(1/2),(w3/(r3))*(1/2),rep(0,sum_m),(w2/(r2+l2))*(1/5),rep(0,sum_l)), byrow = TRUE, nrow = 1);
  Const_1_i3 <- matrix(c(1,rep(0,(K*N)),(w1/(r1+l1))*(1/7),(w1/(r1+l1))*(1/4),(w2/(r2+l2))*(1/5),(w2/(r2+l2))*(1/4),(w3/(r3))*(1/7),(w3/(r3))*(1/1),rep(0,sum_m),(w2/(r2+l2))*(1/2),rep(0,sum_l)), byrow = TRUE, nrow = 1);

  obj_func_i1 <- matrix(c(1,rep(0,(K*N)),rep(0,sum_r),-(w1/(m1))*(1/7),-(w1/(m1))*(1/2),-(w2/(m1+l1))*(1/5),-(w2/(m1+l1))*(1/4),-(w3/(m2+l2))*(1/7),-(w3/(m2+l2))*(1/5),rep(0,sum_l),-(w3/(m2+l2))*(1/1)), byrow = TRUE, nrow = 1);
  obj_func_i2 <- matrix(c(1,rep(0,(K*N)),rep(0,sum_r),-(w1/(m1))*(1/4),-(w1/(m1))*(1/3),-(w2/(m1+l1))*(1/2),-(w2/(m1+l1))*(1/3),-(w3/(m2+l2))*(1/3),-(w3/(m2+l2))*(1/1),rep(0,sum_l),-(w3/(m2+l2))*(1/5)), byrow = TRUE, nrow = 1);
  obj_func_i3 <- matrix(c(1,rep(0,(K*N)),rep(0,sum_r),-(w1/(m1))*(1/1),-(w1/(m1))*(1/8),-(w2/(m1+l1))*(1/5),-(w2/(m1+l1))*(1/7),-(w3/(m2+l2))*(1/6),-(w3/(m2+l2))*(1/4),rep(0,sum_l),-(w3/(m2+l2))*(1/2)), byrow = TRUE, nrow = 1);
  ...
  #Constraint 5:
  Const_5_i1 <- matrix(c(-1,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),-1,rep(0,1),
                         -1,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),rep(0,1),1), byrow = TRUE, nrow = 2);

  Const_5_i2 <- matrix(c(-5,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),-1,rep(0,1),
                         -5,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),rep(0,1),1), byrow = TRUE, nrow = 2);

  Const_5_i3 <- matrix(c(-2,rep(0,N),1,5,2,rep(0,N),rep(0,sum_r),rep(0,sum_m),-1,rep(0,1),
                         -2,rep(0,((N-1)*K)),1,5,2,rep(0,sum_r),rep(0,sum_m),rep(0,1),1), byrow = TRUE, nrow = 2);
